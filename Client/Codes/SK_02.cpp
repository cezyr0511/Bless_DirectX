#include "stdafx.h"
#include "SK_02.h"
#include "Fire_Effect01.h"

#include "Effect_Mgr.h"
#include "Export_Engine.h"

CSK_02::CSK_02( LPDIRECT3DDEVICE9 pGraphicDev )
: CSkill(pGraphicDev)
, m_pRendererCom(NULL)
...
HRESULT CSK_02::Ready_GameObject(_vec3 vPos)
{
	if(FAILED(Ready_Component()))
		return E_FAIL;
	
	m_pTransformCom->Set_Infomation(Engine::CTransform::INFO_POSITION, vPos.x, vPos.y , vPos.z);
	m_pTransformCom->Set_Infomation(Engine::CTransform::INFO_SCALE, m_fScale, 0.03f, m_fScale);

	return S_OK;
}

_int CSK_02::Update_GameObject( const _float& fTimeDelta )  
{
	...
	{
		CEffect* pEffect = (CEffect*)CFire_Effect01::Create(m_pGraphicDev, *m_pTransformCom->Get_vPos());
...
_int CSK_02::LastUpdate_GameObject(const _float& fTimeDelta)
{
	...
	if(m_isColl && m_fLifeTime < 0.15f)
		return -1;
	
	return 0;
}

void CSK_02::Render_GameObject( void )
{
...
HRESULT CSK_02::Ready_Component( void )
{
...
HRESULT CSK_02::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
...
CSK_02* CSK_02::Create( LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CSK_02*	pInstance = new CSK_02(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vPos)))
	{
		MSG_BOX("CSK_02 Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CSK_02::Free( void )
{
